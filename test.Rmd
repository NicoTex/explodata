...
### Data specimen (tous les spÃ©cimens des espÃ¨ces endÃ©miques du Gabon)
...
### Data species (metadata sur les espÃ¨ces endÃ©miques du Gabon)
...
### Data full Gabon (tous les spÃ©cimens du Gabon)
full_gabon <- 
...
grid <- st_read("Shapefiles/Grid/Gabon_herbiers_2_0.25_200_50/Gabon_herbiers_full_2018.shp")
# plot (grid$geometry)
...
# prévoir d'enlever les ined ! + de mettre les noms updatés (Name New) #
...
end_esp_all <- species %>%
  ...
  select ("Endemism status", FAMILY, SPECIES, IUCN_cat) %>%
...
Plot 1 : Nombre de spécimen d'herbiers par espèce endémique
```{r nb specimens, echo=FALSE}
...
Plot 2 : Proportion des habitus chez espèces endémiques
```{r habitus stat, echo = FALSE}
...
Plot 3: Proportion des familles chez espèces endémiques
Plot 4: Famille ayant le plus de % d'espèces endémique (avec nombre d'espèce endémique indiqué)
Plot 5: Genres les plus représentés chez espèces endémiques
```{r taxo stat, echo = FALSE}
...
Plot 6: Proportion des catégories UICN chez espèces endémiques
Plot 7: Proportion des habitus dans catégories UICN des espèces endémiques
...
ggplot(data=end_esp_anal, aes(x= factor(IUCN_cat, levels=c("EX","CR","EN","VU","NT","LC","DD")), fill=`Habitus 1`)) + geom_bar(stat="count") + xlab("IUCN categries") + ylab("Species number") + geom_vline(xintercept=4.5, linetype="dashed", size= 1) + guides(fill=guide_legend(title="Habitus"))
...
```{r threatened taxo stat, echo= FALSE}
...
ggplot(data=threat_fam, aes(x=reorder(FAMILY,-n), y=n)) + geom_bar(stat="identity") + xlab("Families") + ylab("Species number") + geom_text(aes(label=paste(round(n/sum(n)*100,1),"%",sep=" ")), vjust=1.6, color="white") + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.3))
...
ggplot(data=threat_gen, aes(x=reorder(GENUS,-n), y=n)) + geom_bar(stat="identity") + xlab("Genera") + ylab("Species number") + geom_text(aes(label=paste(round(n/sum(n)*100,1),"%",sep=" ")), vjust=1.6, color="white") + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.3))
