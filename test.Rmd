...
spec_grid <- left_join(as_tibble(grid_gab), select(as_tibble(a), ID_grid, n), by = "ID_grid") %>% # integrate nb of specimens per cell to the grid data
  mutate (n = as.numeric(n)) %>% 
  rename (n_spec_full = n)

k = 50 # value of minimal number of specimen in a grid cell to compute resampling
rep = 100
...
threat <- vector()
  if (!is.na (spec_grid$n_spec_full[i]) & spec_grid$n_spec_full[i]>k) {
    for (j in 1 : rep){
      ...
      subsamp <- enframe(subsamp) %>% rename (NameNoAuthors = value) # enframe is the new equivalent of as_tibble for a vector
      end[j] <- nrow(unique(subsamp[subsamp$NameNoAuthors %in% end_esp_all$SPECIES,])) # endemism richness
